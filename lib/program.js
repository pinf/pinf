...
//	}));
	...
	self.flow.add(new FLOW.Task("distribute", function(context, callback) {
		...
		function ensureRemoteExists(callback) {
			if (!context.packageArchivePath) return callback(null);
			return context.adapterMethods.package.isArchiveSynced("aws-s3", function(err, isAdded, notifyAdded) {
				if (err) return callback(err);
				if (isAdded) return callback(null);
				return require.async("../adapters/distribute/aws-s3.js", function(adapter) {
					return adapter.for(context).distribute(function(err) {
						if (err) return callback(err);
						return notifyAdded(context);
					});
				}, callback);
			});
		}

		return ensureRemoteExists(function(err) {
			if (err) return callback(err);
