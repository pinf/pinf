...
exports.for = function(module) {
	...
	exports.parsePointerUri = function(uri) {
		...
		info.id = [uriParsed.hostname].concat(pathnameParts).join("/");
		if (/^\d+(?:\.|$)/.test(info.selector)) {
			info.uid = info.id + "/" + info.selector.replace(/^(\d+)\..*$/, "$1");
		} else
		if (!/\./.test(info.selector)) {
			info.uid = info.id + "/" + info.selector;
			info.did = info.uid.replace(/\//g, "~");
		}
		return info;
	}
