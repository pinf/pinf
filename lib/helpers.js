...
exports.for = function(module) {
	...
	exports.parsePointerUri = function(uri) {
		...
		if (versionStream) {
			info.streamid = info.uid + "/" + versionStream;
		}
		info.did = info.id.replace(/\//g, "~");
		info.dstreamid = info.streamid.replace(/\//g, "~");
		return info;
	}
			...
			return main(function(err) {
				if (err) {
					function logError(err) {
						if (err.previous) logError(err.previous);
						if (err.stack) {
