...
var Context = exports.Context = function(context) {
	...
	// To set a PINF argument use:
	//   * `export PINF_ARGS="---<argumentName>"; pinf ...`
	//   * pinf ---<argumentName> ...`
	function checkArgument(propertyName, argumentName, defaultValue) {
		function findArgument() {
			if (
				(process.env.PINF_ARGS && process.env.PINF_ARGS.indexOf("---" + argumentName) !== -1) ||
				(process.argv.indexOf("---" + argumentName) !== -1)
			) {
				if (process.env.DEBUG === "debug") {
					console.log("| debug | " + "Found".green + " PINF argument `" + "---" + argumentName + "`");
				}
				return true;
			}
			if (process.env.DEBUG === "debug") {
				console.log("| debug | Ignored PINF argument `" + "---" + argumentName + "`");
			}
			return false;
		}
		self[propertyName] = context[propertyName] || (findArgument() ? defaultValue : !defaultValue);
	}
	...
	checkArgument("silent", "silent", false);
	...
	if (process.env.DEBUG === "debug") {
		console.log("| debug | Initializing context for root path: " + self.rootPath);
	}
	...
	self.packagesPath = (context.packagesPath && self._relpath(context.packagesPath)) || "./packages";
	...
	// SCM Related
	self.scm = context.scm || null;
	self.dirty = context.dirty || null;
	checkArgument("ignoreScmDirty", "ignore-scm-dirty", true);

	// Package info once we have a descriptor.
	self.version = context.version || null;

	// More package related properties.
	self.packageArchivePath = context.packageArchivePath || null;
...
Context.prototype.setStream = function(type, stream) {
	// Hide the stream properties from serialization and display.
	this.__proto__[type] = stream;
}

Context.prototype.getPackageId = function() {
	return this.uid.toString("dirname") + "@" + this.version.toString();
}
