	...
	return context.ready(function(err) {
		...
		function bootScriptPathForQuery(query, callback) {
			...
			function checkIfProgram(callback) {
				...
				function checkIfAbsoluteProgramPath(callback) {
					if (!/^\//.test(query)) return callback(null, null);
					return FS.exists(PATH.join(query, "package.json"), function(exists) {
						if (exists) return callback(null, PATH.join(query, "package.json"));
						return callback(null, null);
					});
				}

				return checkIfAbsoluteProgramPath(function(err, bootScriptPath) {
					if (err) return callback(err);
			...
			return checkIfUri(function(err, bootScriptPath) {
				...
				return checkIfProgram(function(err, bootScriptPath) {
					...
					return checkIfCommand(function(err, bootScriptPath) {
						...
						context.logger.console.setup("Create a `package.json` file!");
		...
		function determineProgramPath() {
			if (process.argv.length < 3) {
				return process.cwd();
			}
			return process.argv.slice(2).filter(function(arg) {
				// Remove all arguments starting with `-` as they are PINF or program arguments and not paths.
				return !/^-/.test(arg);
			}).shift() || process.cwd();
		}

		return bootScriptPathForQuery(determineProgramPath(), function(err, bootScript) {
			if (err) return callback(err);

			var program = new PROGRAM.Program(bootScript);

			context.setStream("stdin", stdin);
			context.setStream("stdout", stdout);
